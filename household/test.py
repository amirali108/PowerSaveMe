    ...
    def run_simulation(self):
        for house in self.houses:
            for index,row in self.hourly_data.iterrows():
                house.calculate_total_hourly_consumption(index, row['Price']/100, row['Temperature'], row['Wind speed'], row['Cloudiness'], self.calculate_daily_average_price(index))
                self.randomlst.append(row['Price']/100)
